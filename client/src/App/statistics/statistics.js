...
export const Statistics = props => {
   ...
   const[doubleTapSymbols, setDoubleTapSymbols] = useState([]);
    ...
    const[currStatistic, setCurrStatistic] = useState([10,20,100,77,50,90.60,40]);
    const[currLable, setCurrLable]= useState(['a', 'b', 'c', 'd', 'e','f','g','h']);
    const[yAxisLength, setYAxisLength] = useState(115);
    const [barChartWidth, setBarChartWidth] = useState();
    const[alternativeXAxis, setAlternativeXAxis] = useState(['a.)', 'b.)', 'c.)','d.)', 'e.)','f.)', 'g.)', 'h.)','i.)', 'j.)','k.)', 'l.)', 'm.)','o.)', 'p.)','q.)', 'r.)', 's.)','t.)', 'u.)', 'v.)','w.)', 'x.)']);
    ...
    function createLables(currLables){
        ...
        for(var i = 0; i< lablesToChange.length; i++){
                if(lablesToChange[i].length>4){
                    ...
                    currUndertitle.push(letters[alternativeLablesCounter] + ' ' + lablesToChange[i]);
        ...
        const timer = setInterval(() => {
            ...
            Axios.get(`http://localhost:3001/getAllStatisticSingle/${value}`,).then((response)=>{
              ...
              //  setSingleTapSymbols(response.data[1]);
                ...
                Axios.get(`http://localhost:3001/getAllStatisticDouble/${value}`,).then((response)=>{
         ...
         Axios.get(`http://localhost:3001/getAllStatisticTimeToTap/${value}`,).then((response)=>{
           ...
           console.log('TIME TO TAP', response.data[0],xAxisTTT);
           
         });

         Axios.get(`http://localhost:3001/getAllStatisticFiveTimes/${value}`,).then((response)=>{
            setFiveTimes(response.data[0]);
          
           console.log('TIME TO TAP', response.data[0]);
           
         });
          Axios.get(`http://localhost:3001/getAllStatisticCoding/${value}`,).then((response)=>{
            ...
            <div id="container">
                ...
                <div id= "rightStat">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" id="sel1Stat" onChange={handleChange}>
...
{showStatistic &&
            ...
            <CallBarchart data={currStatistic}
                          ...
                          <br></br>
                         {showLable && <div>Games sorted by actuality (1 = newest game, 20 = oldest game)</div>}
                         {showAlternativeSingle && <div>  {xSingle[0]} {xSingle[1]} </div>}
                         {showAlternativeShift && <div>{xShift[0]} {xShift[1]} {xShift[2]} {xShift[3]} {xShift[4]} <br></br> {xShift[5]} {xShift[6]} {xShift[7]} {xShift[8]} <br></br>
                         {xShift[9]}  {xShift[10]} {xShift[11]} {xShift[12]} <br></br>{xShift[13]} {xShift[14]} {xShift[15]} {xShift[16]}<br></br> {xShift[17]} {xShift[18]}  {xShift[19]} {xShift[20]} <br></br> {xShift[21]} {xShift[22]} {xShift[23]} </div>}
                            {showAlternativeSwitch && <div>{xSwitch[0]} {xSwitch[1]} {xSwitch[2]} {xSwitch[3]} {xSwitch[4]}   </div>}
            </div>}
        </div></div>
    );
};