...
export const RepeatFive = props => {
 ...
 const[showTimerPerWord, setShowTimerPerWord] = useState([false,false,false,false,false]);
 ...
 const[countWrongKey,setCountWrongKey] = useState(0);
  ...
  const[neededTime, setNeedetTime] = useState([]);
   ...
   function clickStart(){
    ...
    console.log('SICK', curShowenKEys);
   ...
   /* var nextW = word[random][0];
    console.log('XXX',curShowenKEys, word[random][0]);

    setKeyToHit(nextW);
    var indexV = gameSymbols.indexOf(nextW);

    
    if(nextW !== -1){
      var keyLen =  gameKeys[indexV];
    
     if(keyLen.length > 1){
       console.log('IIIIII')
     
        setKeyToHit(keyLen);
        nextShiftKeyCombi(keyLen);
       setMultipleKey(true);
   ...
   function finishGame(){
  //  sendUserTracking(value, 'end Game', 'finished Game', ' TI5T ' ); 
   ...
   sendUserTracking(value, 'Score Round',  accuracySend +  '% : ' +score, ' TI5T ' ); 
  Axios.post('http://localhost:3001/FiveTimesResult',
  ...
  sendUserTracking(value, 'All Key Pressed "'+allPressedKeys + '"','SHowen Words "'+ showenKeys +'"', ' TI5T ' ); 
  sendUserTracking(value, 'Typing Speed "'+ typingSPeedArr + '"','SHowen Words "'+ showenKeys +'"', ' TI5T ' ); 
  ...
  function nextSmybol(){
   ...
   //   sendUserTracking(value, 'Tapped One Word',  currAlreadyTapped +  ' : ' +timerShowen, ' TI5T ' ); 
    ...
    console.log('FOUND', gameSymbols[currIndexAppear], gameSymbols.indexOf(':'));
    //---------------------------Multiple Keys----------------------------
      
  
   
    setKeyToHit(gameSymbols[currIndexAppear]);
    console.log('FOUND', gameSymbols[currIndexAppear], currIndexAppear);
   ...
   // sendUserTracking(value, 'Current Accuracy',  currAcc +  '% : Symbols Appeard: ' + symbolsAppeared , ' TI5T ' ); 
    ...
    function downHandler({ key }) {
      ...
      if(gameState === 'play' && dialog === false && pauseDialog===false){
        ...
        if(multipleKey){
         ...
         // sendUserTracking(value, 'Hit Correct Key: ',  key , ' TI5T ' ); 

          nextSmybol();
          setCountWrongKey(0);

        }else{
         // sendUserTracking(value, 'Hit Wrong Key',  'Key Pressed: ' + key +  'Acutale Key: ' +wordToTap[0] , ' TI5T ' ); 
      ...
      if(key === 'r'){
        sendUserTracking(value, 'key press: ' +key, 'Replay', 'Pause-Dialog TI5T ' ); 
        //replay game
        replay();
        
      }else if(key === 'u'){
      //Back
      sendUserTracking(value, 'key press: ' +key, 'End Pause', 'Pause-Dialog TI5T ' );
      setGameState('countdown');
      setPauseDialog(false);
      setCountDown(true);
    }else if(key === 'Backspace'){
      sendUserTracking(value, 'key press: ' +key, 'Back', 'Pause-Dialog TI5T ' );
     ...
     //replay
      if(key === 'r'){
        sendUserTracking(value, 'key press: ' +key, 'Replay', 'End-Dialog TI5T ' );
        replay();

        //Back
      }else if(key === 'Backspace'){
        sendUserTracking(value, 'key press: ' +key, 'Back', 'End-Dialog TI5T ' );
        window.location.href= '/Learn2Tap/GamesMenu';
      }
 ...
 useEffect(() => {
  ...
  const timer = setInterval(() => {
    ...
    if(initialize){
      ...
      Axios.get(`http://localhost:3001/getFiveTimesWords`,).then((response)=>{
      ...
      Axios.get(`http://localhost:3001/getFiveTimeRecords/${value}`,).then((response)=>{
...
// clearing interval
  return () => clearInterval(timer);
    ...
    return(
      <div id="returnMenuDiv">
        ...
        <div width="100%"  class="row">
           ...
           <button class="normalButton" type="button" id="menuButtonSingleTap"
                onClick={() => {
                  sendUserTracking(value, 'button click', 'Menu Button', 'Games TI5T Menu' );  
                  window.location.href=  '/Learn2Tap/GamesMenu'; 
                }
                } 
                >Menu
         ...
         <div id="pauseDiv">
            ...
            onClick={() => {
              sendUserTracking(value, 'button click', 'Pause Button', 'Games TI5T ' );  
              setGameState('pause');
              setPauseDialog(true);
              setShowLetter(false);
        ...
        <div class = "showLetterTutoriel" id="showLetterGames">
          ...
          <div id="prac" style={{color:'black'}}> 
            <div id='symbolDiv'>
              ...
              {normalWord &&  <div><h3 className='word' style={{color:'blue'}}> {alreadyTapped}</h3><h3 className='word'>{wordToTap}</h3></div>}
               
               {spaceWord && <div> <h3 className='word' style={{color:'blue'}}> {alreadyTapped}</h3> &nbsp; <h3 className='word'>{wordToTap}</h3> </div>}
                  <br></br>
                  <br></br>
                  {showTimerPerWord[0] && <div>1. {arrTimePerWord[0]}</div>}
                  {showTimerPerWord[1] && <div>2. {arrTimePerWord[1]}</div>}
                  {showTimerPerWord[2] && <div>3. {arrTimePerWord[2]} </div>}
                  {showTimerPerWord[3] && <div>4. {arrTimePerWord[3]} </div>}
                  {showTimerPerWord[4] && <div>5. {arrTimePerWord[4]} </div>}
      ...
      onClick={() => {
        sendUserTracking(value, 'button click', 'Replay Button', 'End-Dialog TI5T ' );  
       replay();
     ...
     {replaySmbol}
      ...
      <button class="normalButton" type="button" id="dialogButtonGames"
      onClick={() => {
        sendUserTracking(value, 'button click', 'Back Button', 'End-Dialog TI5T ' );  
...
<button class="normalButton" type="button"  id="dialogButtonGames"
   onClick={() => {
    sendUserTracking(value, 'button click', 'Start Button', 'Paus-Dialog TI5T ' );  
     setGameState('countdown');
     setPauseDialog(false);
     setCountDown(true);
   ...
   <button class="normalButton" type="button"  id="dialogButtonGames"
   onClick={() => {
    sendUserTracking(value, 'button click', 'Replay Button', 'Paus-Dialog TI5T ' ); 
    replay();
...
<button class="normalButton" type="button"  id="dialogButtonGames"
   onClick={() => {
    sendUserTracking(value, 'button click', 'Back Button', 'Paus-Dialog TI5T ' ); 
