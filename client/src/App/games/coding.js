...
import { addStatisticToDatabase, sendUserTracking } from "../functions/functions";
...
const[recordStreak, setRecordeStreak] = useState('');
  ...
  const[arrTimesWrong, setArrTimesWrong] = useState([]);
   ...
   const[numWordTapped, setNumWodTapped] = useState(0);
    ...
    const[word,setWord] = useState(['h a l l o', 'kakobuttter', 'ikarus', 'berghain']);

  const[timerPerWOrd, setTimerPerWord] = useState(0.00)
 ...
 const[countWrongKey,setCountWrongKey] = useState(0);
  ...
  const play = useKeyPress();
    ...
    const handleChange=(e)=>{
      ...
      sendUserTracking(value, 'Select Num ROunds', 'Rounds: ' + numberRounds, 'End-Dialog Code' );
   ...
   function nextShiftKeyCombi(combi){
    ...
    console.log('FOUND SHIFT', singleShiftKeys, shiftKeysPressed);
  ...
  function createShiftObject(keyPressed){
    console.log('FOUND CREATE', readShiftKey, currShiftKeys);

    var res = readShiftKey;
    var currSingleKeys = currShiftKeys;
  
   console.log('CREATE ',keyPressed, currSingleKeys );
   ...
   function finishGame(){
    //sendUserTracking(value, 'games finishes', 'game finished', 'CodeGame' ); 
     
     setGameState('endDialog');

     var currAcc = parseInt( ((rightHitTotal+1)/(wrongHitTotal + (appearSymbolTotal+1)))*100);
    ...
    for(var i = 0; i<arrayTimesLetterAp.length; i++){
      if(arrayTimesLetterAp[i] >0){
        ...
        var calculate = (arrayTimesLetterHit[i]/(arrayTimesLetterAp[i]+ arrTimesWrong[i]))*100;
    ...
    //  addStatisticToDatabase(value, singleTapSymbols, singleRes, 'U0_L0', 0, 'singleTap');
      Axios.post('http://localhost:3001/statisticSingleTapWReponse',
      ...
      console.log('Reponse 1', response.data[0]);
    ...
    // addStatisticToDatabase(value, doubleTapSymbols, doubleRes, 'U0_L0', 0, 'doubleTap');
    ...
    // addStatisticToDatabase(value, tripleTapSymbols, tripleRes, 'U0_L0', 0, 'tripleTap');
   ...
   //addStatisticToDatabase(value, switchSymbols,switchRes, 'U0_L0', 0, 'switch');
      ...
      console.log('DOne send result')
      var accuracySend =   parseInt( ((rightHitTotal+1)/(wrongHitTotal + (appearSymbolTotal+1)))*100);
      var score =  hitCounter + 1;
     
     Axios.post('http://localhost:3001/CodingResult',
...

  {
    ...
    setDialog(true);
    //sendUserTracking(value, 'games finished - score', accuracySend + ' % , Score: '+ score, 'CodeGame' ); 
    sendUserTracking(value, 'All Key Pressed "'+allPressedKeys + '"','SHowen Words "'+ showenKeys +'"', 'CodeGame' ); 
    sendUserTracking(value, 'Typing Speed "'+typingSPeedArr + '"','SHowen Words "'+ showenKeys +'"', 'CodeGame' ); 
   ...
   function splitWord(wordToSplit){
    ...
    setShowenKey(currShowenKey);
     ...
     for(var i = 0; i<currLines.length; i++){
      ...
      console.log('FOUND', gameSymbols )
      console.log('FOUND FUNCTION',stillToTapT, nextToTapS, indexNextSymb);
    
      
      setKeyToHit(nextToTapS);
      if(indexNextSymb !== -1){
        var lengthOfKeyS =  gameKeys[indexNextSymb];
        console.log('FOUND SPLIT', lengthOfKeyS);
      ...
      console.log('FOUND CUR', currWordArry[0]);
  ...
  function nextSmybol(){
   ...
   console.log('FINSIH ROUND' , newWordToTap.length === 0,  testFin.length === 1, numberOfRounds-1 > roundsPlayed);
  
   if(newWordToTap.length === 0 && testFin.length === 1 && numberOfRounds-1 > roundsPlayed){
      wordGetNotSplit = false;
      console.log('FINSIH ROUND');
    ...
    }else if(numberOfRounds-1 === roundsPlayed && newWordToTap.length === 0 && testFin.length === 1 ){
      wordGetNotSplit = false;
      console.log('FINSIH');
     // setGameState('');
      setShowLetter(false);
   ...
   //  setHitCounter(hitCounter +  currAlreadyTapped.length);
    ...
    if(stillToTapN[0] == undefined){
      ...
      console.log('FOUND NOT',   wordToTap[numWordTapped +1 ]);
    }
    ...
    //var currKeyCombi = keysCom[indexNext];
    console.log('FOUND',nextToTap, indexNextSym, lengthOfKey);
   ...
   function useKeyPress() {
    ...
    function downHandler({ key }) {
      ...
      if(gameState === 'play' && dialog === false && pauseDialog===false){
       ...
       if(multipleKey){
        ...
        createShiftObject(key);
         ...
         // sendUserTracking(value, 'keyPress', 'write keyPress: '+ key, 'CodeGame' ); 
          setCountWrongKey(0);
          nextSmybol();
         console.log('FOUND EASY WAY', multipleKey);

        }else{
         // sendUserTracking(value, 'false keyPress', 'write key: '+ thisWord[0] + 'Wrong key: '+ key, 'CodeGame' ); 
          if(hitCounter>0){
            setHitCounter(hitCounter-1);
          }
          setCountWrongKey(countWrongKey +1);
          setWrongHitTotal(wrongHitTotal +1);
          var currAcc = parseInt( ((rightHitTotal)/((wrongHitTotal+1) + (appearSymbolTotal)))*100);
          setAccuracyStr(currAcc + "%");
      ...
      if(key === 'r'){
        sendUserTracking(value, 'key press: ' +key, 'Replay /Start New Game', 'Pause-Dialog Code' );
        //replay game
        replay();
        
      }else if(key === 'u'){
      //Back
      sendUserTracking(value, 'key press: ' +key, 'End Pause', 'Pause-Dialog Code' );
      setGameState('countdown');
      setPauseDialog(false);
      setCountDown(true);
    }else if(key === 'Backspace'){
      sendUserTracking(value, 'key press: ' +key, 'Back', 'Pause-Dialog Code' );
     ...
     //replay
      if(key === 'r'){
        sendUserTracking(value, 'key press: ' +key, 'Replay', 'End-Dialog Code' );
        replay();

        //Back
      }else if(key === 'Backspace'){
        sendUserTracking(value, 'key press: ' +key, 'Back', 'End-Dialog Code' );
 ...
 useEffect(() => {
  ...
  const timer = setInterval(() => {
    if(showTick === false){
    var nextTime =(parseFloat( timerPerWOrd + 0.10));
    ...
    if(initialize){
      ...
      Axios.get(`http://localhost:3001/getCodingRecords/${value}`,).then((response)=>{
    ...
    Axios.get(`http://localhost:3001/getSymbolArraySingleTab/${[27,29]}/${currRight}/${0}`,).then((response)=>{
    ...
    Axios.get(`http://localhost:3001/getSymbolArrayDoubleTap/${[27,31]}/${currRight}/${0}`,).then((response)=>{
    ...
    Axios.get(`http://localhost:3001/getSymbolArrayTripleTap/${[26,27]}/${currRight}/${0}`,).then((response)=>{
  ...
  Axios.get(`http://localhost:3001/getSymbolArraySwitch/${[27,29]}/${currRight}/${0}`,).then((response)=>{
      ...
      <div id="returnMenuDiv">
        ...
        <div width="100%"  class="row">
           ...
           <button class="normalButton" type="button" id="menuButtonSingleTap"
                onClick={() => {
                  sendUserTracking(value, 'button click', 'Menu Button', 'Games Code Menu' );  
                  window.location.href=  '/Learn2Tap/GamesMenu'; 
                }
                } 
                >Menu
         ...
         <div id="pauseDiv">
            ...
            onClick={() => {
              sendUserTracking(value, 'button click', 'Pause Button', 'Games Code ' );  
              setGameState('pause');
              setPauseDialog(true);
              setShowLetter(false);
...
<div className='modeSelection' id='modeSelectionDiv'>
    ...
    <div id="rightCode">
                ...
                <button class="normalButton"  id= "startButtonGames" type="button" 
  onClick={() => {
  ...
  >Start
        ...
        <div class = "showLetterTutoriel">
          ...
          <div id="prac" style={{color:'black'}}> 
            <div id='symbolDivCoding'>
              ...
              {normalWord &&  <div><h3 className='word' style={{color:'blue'}}> {alreadyTapped[0]}</h3><h3 className='word'>{wordToTap[0]}</h3>
                  <br></br> <h3 className='word' style={{color:'blue'}}> {alreadyTapped[1]}</h3><h3 className='word'>{wordToTap[1]}</h3>
                  <br></br> <h3 className='word' style={{color:'blue'}}> {alreadyTapped[2]}</h3><h3 className='word'>{wordToTap[2]}</h3>
                  <br></br> <h3 className='word' style={{color:'blue'}}> {alreadyTapped[3]}</h3><h3 className='word'>{wordToTap[3]}</h3>
              
              
              </div>}
               
               {spaceWord && <div> <h3 className='word' style={{color:'blue'}}> {alreadyTapped[0]}</h3> &nbsp; <h3 className='word'>{wordToTap[0]}</h3> 
               <br></br> <h3 className='word' style={{color:'blue'}}> {alreadyTapped[1]}</h3> &nbsp; <h3 className='word'>{wordToTap[1]}</h3> 
               <br></br> <h3 className='word' style={{color:'blue'}}> {alreadyTapped[2]}</h3> &nbsp; <h3 className='word'>{wordToTap[2]}</h3> 
               <br></br> <h3 className='word' style={{color:'blue'}}> {alreadyTapped[3]}</h3> &nbsp; <h3 className='word'>{wordToTap[3]}</h3> 
                  ...
                  <br></br>
                  <br></br>
                {showSpaceTip && <div> <h6>Next Space</h6></div>}
                <br></br>
                </div>}
                {showTick &&
                <p class="letterTutoriel" style={{color:'green'}}>	✓ </p>}
                {showCross &&
                <p class="letterTutoriel" style={{color:'red'}}>✗</p>}
      ...
      onClick={() => {
        sendUserTracking(value, 'Button CLick', 'Replay', 'End-Dialog Code' );
        replay();
     ...
     {replaySmbol}
      ...
      onClick={() => {
        ...
        sendUserTracking(value, 'Button CLick', 'Back', 'End-Dialog Code' );
...
<button class="normalButton" type="button"  id="dialogButtonGames" 
   onClick={() => {
     ...
     setPauseDialog(false);
     setCountDown(true);
     sendUserTracking(value, 'Button CLick', '▷', 'Pause-Dialog Code' );
   ...
   <button class="normalButton" type="button"  id="dialogButtonGames"
   onClick={() => {
    sendUserTracking(value, 'Button CLick', 'Replay', 'Pause-Dialog Code' );
...
<button class="normalButton" type="button"  id="dialogButtonGames" 
   onClick={() => {
    sendUserTracking(value, 'Button CLick', 'Exit', 'Pause-Dialog Code' );
