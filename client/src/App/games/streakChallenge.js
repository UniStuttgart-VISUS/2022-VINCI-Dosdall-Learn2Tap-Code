...
const[recordStreak, setRecordeStreak] = useState('');
  ...
  const[arrTimesWrong, setArrTimesWrong] = useState([]);
   ...
   function clickStart(){
    ...
    var currCombination = [];
    var currKeys = [];
    sendUserTracking(value, 'selection ','"'+selection+ '"', 'Games-TimeToTap' );  

    if(selection[0]){
     // sendUserTracking(value, 'selection Single Tap',selection[0] , 'Games-TimeToTap' );  
    ...
    if(selection[1]){
   //   sendUserTracking(value, 'selection Double Tap',selection[1] , 'Games-TimeToTap' );  
   ...
   if(selection[2]){
    //sendUserTracking(value, 'selection Triple Tap',selection[2] , 'Games-TimeToTap' );  
      ...
      //  sendUserTracking(value, 'selection Shift',selection[3] , 'Games-TimeToTap' );  
    ...
    if(selection[4]){
  //    sendUserTracking(value, 'selection Switch',selection[4] , 'Games-TimeToTap' );  
  ...
  var lenghtA = currSymbols.length;
   ...
   if(currKeys[currOrder[0]].length > 1){
    ...
    setMultipleKey(true);
     ...
     if(selection[3] && currOrder[0] >= startIndexShift && currOrder[0] <=endIndexShift){
...
function sendUserTrackingAtENd(){
 ...
 console.log('END SCORE',score, currAcc, hitCounter );
 Axios.post('http://localhost:3001/TimeToTapResult',
...
sendUserTracking(value, 'All Key Pressed "'+allPressedKeys + '"','SHowen Words "'+ showenKeys +'"', 'TTT' ); 
sendUserTracking(value, 'SHowen Words "'+ showenKeys +'"','TypingSpeed "'+ typingSPeedArr +'"', 'TTT' ); 
   ...
   function sendDoubleTap(resultDTDT, resultTTDT, resultshiftDT, resultSwitchDT){
   ...
   function sendSingleTapStat(resultST, resultDT, resultTT, resultShift, resultSwitch){
   ...
   function finishGame(){
    ...
    for(var i = 0; i<arrayTimesLetterAp.length; i++){
      ...
      console.log('Single', arrayTimesLetterHit, arrayTimesLetterAp, arrTimesWrong, resArr);
   ...
   if(selection[3]){
    ...
    addStatisticToDatabase(value, shiftSymbols, shiftRes, 'U0_L0', 0, 'shift');
  ...
  function nextSmybol(rightHit){
    ...
    if(selection[3] && indexNext > startIndexShift && indexNext <endIndexShift){
      setShowShiftSymbolToHit(true);
      setShowSymbolToHit(false);
  }else{
    setShowSymbolToHit(true);
   ...
   function useKeyPress() {
    ...
    function downHandler({ key }) {
      ...
      if(gameState === 'play' && showCross ===false && showTick ===false && dialog ==false && pauseDialog==false){

        if(keyToHit.length === 1){
      
        
      if(key === keyToHit){
       // sendUserTracking(value, 'key press: ' +key, 'Tapped write Key', 'Game TTT' );
        nextSmybol(true);
      }else{
        nextSmybol(false);
      }
      ...
      if(key === 'r'){
        sendUserTracking(value, 'key press: ' +key, 'Replay/ Start New Game', 'Pause-Dialog TTT ' );
        //replay game
        replay();
        
      }else if(key === 'u'){
      //Back
      sendUserTracking(value, 'key press: ' + key, 'End pause', 'Pause-Dialog TTT ' );
      setGameState('countdown');
      setPauseDialog(false);
      setCountDown(true);
    }else if(key === 'Backspace'){
      sendUserTracking(value, 'key press: ' +key, 'Back', 'Pause-Dialog TTT ' );
     ...
     //replay
      if(key === 'r'){
        sendUserTracking(value, 'key press: ' +key, 'Replay', 'End-Dialog TTT' );
        replay();

        //Back
      }else if(key === 'Backspace'){
        sendUserTracking(value, 'key press: ' +key, 'Back', 'End-Dialog TTT ' );
 ...
 useEffect(() => {
  ...
  const timer = setInterval(() => {
    if(initialize){
      ...
      Axios.get(`http://localhost:3001/getTimeToHitRecords/${value}`,).then((response)=>{
    ...
    Axios.get(`http://localhost:3001/getSymbolArrayDoubleTap/${[27,31]}/${currRight}/${0}`,).then((response)=>{
     ...
     setDoubleTapSymbols(currSymbolDouble);
      ...
      console.log('DOubleTap', currSymbolDouble);
  ...
  currSymbolTriple[indexToUse] = nextSym;
   ...
   console.log('TripleeTap', currSymbolTriple);
  
  })

  Axios.get(`http://localhost:3001/getSymbolArrayShift/${[26,27]}/${currRight}/${0}`,).then((response)=>{
  ...
  currKeySwitch[indexFirst+1] = ';';
   ...
   setSwitchSymbols(currSySwitch);
    ...
    console.log( response.data[2], response.data[3], response.data[5]);
     ...
     if(currMin <0){
      ...
      if(nextSymTimerRunning){
        ...
        if(timerNextSym === 1){
          ...
          setShowCross(false);
          setShowTick(false);
          if(selection[3] && indexCurrSymbol > startIndexShift && indexCurrSymbol <endIndexShift){
            setShowShiftSymbolToHit(true);
        }else{
...
// clearing interval
  return () => clearInterval(timer);
    ...
    return(
      <div id="returnMenuDiv">
        ...
        <div width="100%"  class="row">
           ...
           <button class="normalButton" type="button" id="menuButtonSingleTap"
                onClick={() => {
                  sendUserTracking(value, 'button click', 'Menu Button', 'Games Menu' );  
                  window.location.href=  '/Learn2Tap/GamesMenu'; 
                }
                } 
                >Menu
         ...
         <div id="pauseDiv">
            ...
            onClick={() => {
              sendUserTracking(value, 'button click', 'Pause Button', 'Games-TimeToTap' );  
              setGameState('pause');
              setPauseDialog(true);
              setShowLetter(false);
            }}
        ...
        {showSelection &&
          ...
          <div className='modeSelection'>
            ...
            <div class="row">
              ...
              <div class="col">
                <input  type="checkbox" className="custom_checkbox" value='0' onChange={handleChange}></input>
                <label for="myInput">Single Tap</label>
              </div>
              <div class="col">
                 <input type="checkbox"class="custom_checkbox" value='1' onChange={handleChange}></input>
                <label for="myInput">Double Tap</label>
              </div>
              <div class="col">
                <input type="checkbox"  className="custom_checkbox" value='2' onChange={handleChange}></input>
                 <label for="myInput">Triple Tap</label>
              </div>
              <div class="col">
                <input  className="custom_checkbox" type="checkbox" value='3'onChange={handleChange}></input>
               <br></br> 
               <label for="myInput">Shift</label>
              </div>
              <div class="col">
                <input  className="custom_checkbox"  type="checkbox" value='4' onChange={handleChange} ></input>
                 <br></br> 
           ...
           <button class="normalButton"  id= "startButtonGames" type="button" 
            onClick={() => {
              sendUserTracking(value, 'button click', 'Start Button', 'Games-TimeToTap' );  
              if(selection.indexOf(true) === -1){
                  alert("You must select at least one mode to start 'Time To Tap'.")
              }else{
              clickStart();
            }}}
            >Start
          ...
          <div id="prac" style={{color:'black'}}> 
            <div id='symbolDiv' >
              ...
              {showSymbolToHit &&
                ...
                {showTick &&
                <p id="letterTutoriel" style={{color:'green'}}>	✓ </p>}
                {showCross &&
                <p id="letterTutoriel" style={{color:'red'}}>✗</p>}
                <br></br>
               <h5> {help}</h5>
               <br></br>
            </div>
      ...
      onClick={() => {
        sendUserTracking(value, 'button click', 'Replay Button', 'TTT-EndDialog' );  
       replay();
     ...
     {replaySmbol}
      ...
      <button class="normalButton" type="button"  id="dialogButtonGames"
      onClick={() => {
        sendUserTracking(value, 'button click', 'Back Button', 'TTT-EndDialog' );  
...
<button class="normalButton" type="button"  id="dialogButtonGames"
   onClick={() => {
    sendUserTracking(value, 'button click', 'Contine Game', 'TTT-PauseDialog' );  
     setGameState('countdown');
     setPauseDialog(false);
     setCountDown(true);
   ...
   <button class="normalButton" type="button"  id="dialogButtonGames"
   onClick={() => {
    sendUserTracking(value, 'button click', 'Replay Game', 'TTT-PauseDialog' ); 
    replay();
...
<button class="normalButton" type="button"  id="dialogButtonGames"
   onClick={() => {
    ...
    sendUserTracking(value, 'button click', 'Exit Game', 'TTT-PauseDialog' ); 
    window.location.href= '/Learn2Tap/GamesMenu';
      }
