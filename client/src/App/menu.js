  ...
  export const Menu = props  =>{
    ...
    function showUser(){
      if(idCookie=== 0){
        ...
        setShowAlert(true);
        setWelcomeMessage('No user logged in.  Go to User Administration to register a user.')
    ...
    useEffect(() => {
      ...
      if(initialize){
        ...
        if( currCookieID.indexOf('u')== 0 ){
          ...
          showUser();
          sendUserTracking(0, 'Not Registered User Open Page', 'Not Registered User Open App', 'menu' ); 
    
        }else{
    
          Axios.get(`http://localhost:3001/getSpecificUserName/${currCookieID}`).then((response)=>{
   ...
   return(  
    ...
    {showMenu &&
      ...
      <button class="normalButton" type="button" id="userAdministrationButton"
        onClick={() => {
          sendUserTracking(value, 'button click', 'user Administration button', 'menu' ); 
        ...
        <div  class="col" id="addSpace" ></div>
        <div  class="col"  >
        <button type="button" class="normalButton" id="hintButton" 
            onClick={() => {
    ...
    <br></br>
        ...
        <strong>Info!</strong> Set your system to US - English to use TapStrap without difficulty.
      </div>
      }
      <p>{welcomeMesssage}</p>
      ...
      <button class="normalButton" type="button" id="glossaryButton"
        onClick={() => {
        ...
        <div class="Practice">
          	...
          	<button id="practiceButton"  onClick= {()=>{
              sendUserTracking(value, 'button click', 'load practice', 'menu' ); 
              loadDoc();}}>Practice</button>
            <button id="delode" onClick= {()=>{
              sendUserTracking(value, 'button click', 'deloade practice', 'menu' ); 
              deloadDoc();}}>^</button>  
          
        </div>
        <div id="demo"></div>
      
        <button class="normalButton"  id= "gameButton" type="button" 
          onClick={() => {
        ...
        <button class="normalButton"  type="button" id="statisticButton"
          onClick={() => {
