   ...
   const[askSpecificUser, setAskSpecificUser] = useState(false)
    ...
    const getSpecificUser = (name) =>{
        ...
        if(response.data[0] === undefined && askSpecificUser){
          ...
          sendUserTracking(value, 'Alert Message', 'Alert - Welcome', 'User Administration' );
      ...
      {userList.map((val, key) => {
          return(
            <div className="userListResult">
              ...
              <div class="half2">
            <button  class="normalButton" id="selectUserButton"
              onClick={() =>  {
                sendUserTracking(value, 'button click', 'Load Data button', 'User Administration' );
