...
export const AEIOULevel1 = (props) => {
	...
	function calculateStatistic() {
			...
			if (arrayTimesLetterAp[i] > 0) {
				statitisc[i] =
					(arrayTimesLetterHit[i] /
						(arrayTimesLetterAp[i] + arrWrongLetterHit[i])) *
					100;
			} else {
				statitisc[i] = 0;
			}
		...
		for (let i = 0; i < statitiscAll.length; i++) {
			if (arrayTimesLetterApAll[i] > 0) {
				var currResStat =
					(arrayTimesLetterHitAll[i] /
						(arrayTimesLetterApAll[i] + arrWrongLetterHitAll[i])) *
					100;
	...
	function newBubble(hit) {
		...
		if (allSymbolArr.includes(symbolsToUse[letterCounter])) {
			...
			if (hit) {
				var indexOfLetterAllHit = allSymbolArr.indexOf(
					symbolsToUse[letterCounter]
				);
		...
		if (numLetterShow > symbolArr.length * 2 && allLetterInUse && level == 1) {
			...
			numLetterShow > symbolArr.length &&
			allLetterInUse &&
			level == 2
		) {
			setSymbolsToUse(allSymbolArr);
		...
		if (
			typingMode !== "singleTap" &&
			lettersShowenRound > orderLetter.length - 1 === false
		) {
	...
	function useKeyPress() {
		...
		function downHandler({ key }) {
			...
			} else if (symbolsToUse.indexOf(key) != -1) {
				setKeyPressed(true);
			...
			if (pauseDialog && state !== "play") {
				if (key === "r") {
					sendUserTracking(
						...
						"Key stroke r",
						"Resume Button",
						"Pause Dialog Level" + unitName + "Level " + level
					);
	...
	useEffect(() => {
		const timer = setInterval(() => {
			if (initializeLevle) {
				...
				var currSymbolsToPrac = clearCookie(
					Cookies.get("symbolArrCookie")
				).length;

				setAllSymbolArr(
					currAllSymbols,
					clearCookie(Cookies.get("symbolArrCookie"))
				);

				setAllCombinationArray(
					clearCookie(Cookies.get("allCombinationArrCookie"))
				);
				...
				if (currTypingMode === "singleTap") {
					...
					Axios.get(
						`http://localhost:3001/getSymbolArrayDoubleTap/${symbolIds}/${rightHand}/${value}`
					).then((response) => {
					...
					Axios.get(
						`http://localhost:3001/getSymbolArrayTripleTap/${symbolIds}/${rightHand}/${value}`
					).then((response) => {
						setSymbolArr(response.data[0]);
						setSymbolsToUse(response.data[0]);
						setCombinationToUse(response.data[1]);
					...
					setEndUrl("ShiftMenu");
					Axios.get(
						`http://localhost:3001/getSymbolArrayShift/${symbolIds}/${rightHand}/${value}`
					).then((response) => {
						setSymbolArr(response.data[0]);
						setSymbolsToUse(response.data[0]);
						setCombinationToUse(response.data[1]);
								...
								<button
									...
									onClick={() => {
										sendUserTracking(
											...
											"button click",
											"Exit Button",
											"Exit Dialog - " + unitName + "Level " + level
										);
										if (levelExist === 2) {
											window.location.href = "/Learn2Tap/MenuPracticeUnit";
										} else {
								...
								<button
									...
									onClick={() => {
										sendUserTracking(
											...
											"button click",
											"Proceed Button",
											"End Dialog " + unitName + "Level " + level
										);
										var currLevel = parseInt(level) + 1;
										setLevel(currLevel);
										Cookies.set("level", currLevel);
								...
								<button
									...
									onClick={() => {
										sendUserTracking(
											...
											"button click",
											"Replay Button",
											"End Dialog Level" + unitName + "Level " + level
										);
										window.location.href = "/Learn2Tap/Level";
									}}
								>
								...
								<button
									...
									onClick={() => {
										sendUserTracking(
											...
											"button click",
											"Replay  Button",
											"Time is up Dialog Level" + unitName + "Level " + level
										);
										window.location.href = "/Learn2Tap/Level";
									}}
								>
