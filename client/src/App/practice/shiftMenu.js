...
export const ShiftMenu = (props) => {
	...
	useEffect(() => {
		...
		Axios.get(`http://localhost:3001/getShiftStar/${userID}`).then(
			(response) => {
				...
				<div>
					...
					<div class="menuDiv">
						<div class="pointDiv" class="row">
							<h5>{pointsTotal}/69</h5>{" "}
							<img src={star} width="60px" height="60px" alt="star" />
						</div>
						<h2>SHIFT</h2>
						<div class="alert" id="alertShift">
							<strong>Info!</strong> 1× {combinationSwitchModus} TapStrap is in
							Shift mode for a following input. <br></br>
							2× {combinationSwitchModus} TapStarp is permanently in Shift mode
							--> Tap {combinationSwitchModus} again to exit the Shift mode.
						</div>
						<div class="alert alert-warning" id="alertShift">
							<strong>Warning!</strong>
							Disable the shortcut function for your browser to practice shift
							mode without problems.
						</div>
						<div class="menuButtons">
							<button
								class="normalButton"
								id="practicButtonMenuShift"
								type="button"
								onClick={() => {
									sendUserTracking(
										value,
