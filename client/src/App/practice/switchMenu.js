...
export const SwitchMenu = props => {
  ...
  function getLevelValues(symbolIds, rightHand, currModus, currUnit, unitID ,firstUrl){
    ...
    useEffect(() => {
      ...
      Axios.get(`http://localhost:3001/getSwitchStar/${userID}`,).then((response)=>{
      ...
      return(
        ...
        <div>
            ...
            <div width="100%"  class="row">
             ...
             <button class="normalButton" type="button" id="menuButtonSingleTap"
                  onClick={() => {
                    sendUserTracking(value, 'button click', 'Menu Button', 'Switch Menu' );  
              ...
              <button class="normalButton" type="button" id="glossaryButtonSingleTapMenu"
                  onClick={() => {
                    sendUserTracking(value, 'Button click', 'Glossary Button', 'Switch Menu' ); 
        ...
        <strong>Info!</strong> To activate the switch mode tap: {combinationSwitchModus}.
      </div>
              <div class="menuButtons">
              <button class="normalButton" id="practicButtonMenuSwitch" type="button" 
            onClick={() => {
              sendUserTracking(value, 'Button click', levelName[0] + 'Button', 'Switch Menu' ); 
          ...
          <button class="normalButton" id="practicButtonMenuSwitch" type="button" 
            onClick={() => {
              sendUserTracking(value, 'Button click', levelName[1] + 'Button', 'Switch Menu' ); 
          ...
          <button class="normalButton" id="practicButtonMenuSwitch" type="button" 
            onClick={() => {
              sendUserTracking(value, 'Button click', levelName[2] + 'Button', 'Switch Menu' ); 
              Cookies.set('symbolIDsArray',[15,14,12,13]);
              getLevelValues([15,14,12,13], rightHand, levelDescription[2],levelName[2], 3, false );
          ...
          <button class="normalButton" id="practicButtonMenuSwitch" type="button" 
            onClick={() => {
              sendUserTracking(value, 'Button click', levelName[3] + 'Button', 'Switch Menu' ); 
              Cookies.set('symbolIDsArray',[17,19,21,22]);
              getLevelValues([17,19,21,22], rightHand, levelDescription[3],levelName[3], 4, false );
          ...
          <button class="normalButton" id="practicButtonMenuSwitch" type="button" 
            onClick={() => {
              sendUserTracking(value, 'Button click', levelName[4] + 'Button', 'Switch Menu' ); 
              Cookies.set('symbolIDsArray',[23,24]);
              getLevelValues([23,24], rightHand, levelDescription[4],levelName[4], 5 , false);
          ...
          <button class="normalButton" id="practicButtonMenuSwitch" type="button" 
            onClick={() => {
              sendUserTracking(value, 'Button click', levelName[5] + 'Button', 'Switch Menu' );
              Cookies.set('symbolIDsArray',[26,27,29]); 
              getLevelValues([26,27,29], rightHand, levelDescription[5],levelName[5], 6, false );
